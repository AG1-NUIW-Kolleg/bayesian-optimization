...
# parse arguments
rank_no = int(sys.argv[-2]) # ist in variables.py in cuboid enthalten
n_ranks = int(sys.argv[-1]) # ist in variables.py in cuboid enthalten

variables.n_subdomains = variables.n_subdomains_x*variables.n_subdomains_y*variables.n_subdomains_z

if variables.n_subdomains != n_ranks: # nur fehlermeldung, sollte nicht relevant sein
...
def n_fibers_in_subdomain_x(_): # nur funktion, sollte keinen Unterschied machen
    return variables.n_fibers_x
def n_fibers_in_subdomain_y(_): # nur funktion, sollte keinen Unterschied machen
    return variables.n_fibers_y
def get_fiber_no(subdomain_coordinate_x, subdomain_coordinate_y, fiber_in_subdomain_coordinate_x, fiber_in_subdomain_coordinate_y): # wird so in variables.py im cuboid definiert
    return fiber_in_subdomain_coordinate_x + variables.n_fibers_x * fiber_in_subdomain_coordinate_y

variables.n_subdomains_xy = variables.n_subdomains_x * variables.n_subdomains_y # wofür?
variables.n_fibers_total = variables.n_fibers_x * variables.n_fibers_y # wofür?

#############################
# create the partitioning using the script in create_partitioned_meshes_for_settings.py -> wo ist dieses File?
# 3D meshes are created internally
# however, we have to create the fibers on our own

# fiber directions
fiber_meshes = {} # warum mehrfach meshes definiert?
fiber_mesh_names = []

### muscle 1
for j in range(variables.n_fibers_y):
  for i in range(variables.n_fibers_x):
    fiber_no = j*variables.n_fibers_x + i #über get_fiber_no definiert, get_fiber_no in diesem file zwar als Funktion definiert, aber nicht verwendet
    
    # determine start position of fiber in (x,y)-plane
    x = 0 + i / (variables.n_fibers_x - 1) * variables.muscle_extent[0] # auch in cuboid
    y = 0 + j / (variables.n_fibers_y - 1) * variables.muscle_extent[1] # auch in cuboid
    ...
    mesh_name = "muscle1_fiber{}".format(fiber_no) # wird in cuboid ohne 1 im Namen für einen Muskel übersetzt
...
#### set output writer
variables.output_writer_fibers_muscle2 = [] #TODO
variables.output_writer_emg_muscle2 = [] #TODO
...
if "hodgkin_huxley" in variables.cellml_file and "hodgkin_huxley-razumova" not in variables.cellml_file:
...
elif "shortened" in variables.cellml_file:
   # parameters: stimulation current I_stim, fiber stretch λ
   variables.muscle1_mappings = {
     ("parameter", 0):           ("algebraic", "wal_environment/I_T"), # parameter is algebraic 32
  ...
  variables.vm_value_stimulated = 20.                               # aus cuboid mit 20 übernommen unter name value_of_stimulated_point
...
variables.fiber_distribution = np.genfromtxt(variables.fiber_distribution_file, delimiter=" ", dtype=int) # wird in cuboid im settings_muscle_contraction geladen
variables.firing_times = np.genfromtxt(variables.firing_times_file) # wird in cuboid im settings_muscle_contraction geladen
...
def get_diffusion_prefactor(fiber_no, mu_no): # wird im cuboid direkt ausgerechnet
